    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                fontScale = 1
                cv2.putText(im0, 'Total = ' + str(count2), org, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'Mobil = ' + str(car2), org1, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'Truk and Bus = ' + str(truck2), org2, font,
                            fontScale, color, thickness, cv2.LINE_AA)
...
def count_obj1(box, w, h, id):
    global count2, data, t7, t6, truck2, car2,person
    center_coordinates = (
    ...
    if int(box[1]+(box[3]-box[1])/2) > (240) and int(box[1]+(box[3]-box[1])/2) < (310):
        if id not in data:
            ...
            #     body = """
            #     ON RAMP ALAUDDIN 1
            #     Total Kendaraan """+str(count2)+"""
            #     Total Mobil """""+str(car2)+"""
            #     Total Truck """""+str(truck2)+"""
            #     """
            #     # SMPTP
...
def count_pelanggaran(box, w, h, id, dir2, im0, gambar, simpan):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (240) and int(box[1]+(box[3]-box[1])/2) < (300) and int(box[0]+(box[2]-box[0])/2) < (390) and int(box[0]+(box[2]-box[0])/2) > (270):
        if id not in data2:
            ...
            waktu4 = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            inputpelanggaran(jp, gambar, dir2, simpan)
            
            print("-------------------------------------------------------")
            print("Selain Kendaraan Roda 4 Memasuki Tol - On Ramp Alauddin")
            ...
            try :
                ...
                pywhatkit.sendwhats_image(
                    ...
                    "\n Jenis Pelanggaran : Selain Kendaraan Roda 4 Memasuki Tol"+\
                    "\n Lokasi : On Ramp Alauddin"+\
                    "\n Waktu : "+str(waktu4)+"",
                    tab_close=True)
                pg.press("enter")
                ...
                print("Notifikasi Whatsapp berhasil dikirim pada "+str(waktu4))
                
                for x in receiver_id:
                    bot.sendPhoto(x, photo=open(dir2, 'rb')) # send message to telegram
                    bot.sendMessage(x, ' Selain Kendaraan Roda 4 Memasuki Tol di On Ramp Alauddin') 
...
# def inputdata(car2, truck2, count2):
#     car = str(car2)
#     truck = str(truck2)
#     count = str(count2)
#     lokasi = 'On Ramp Alauddin'
