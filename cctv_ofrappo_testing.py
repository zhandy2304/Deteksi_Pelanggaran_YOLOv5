...
def gen(opt):
    ...
    out, source6, yolo_model, deep_sort_model, show_vid, save_vid, save_txt, imgsz, evaluate, half, project, name, exist_ok = \
        opt.output, opt.source6, opt.yolo_model, opt.deep_sort_model, opt.show_vid, opt.save_vid, \
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                # cv2.line(im0, start_point2, end_point2, color1, thickness=1)
                # cv2.line(im0, start_point3, end_point3, color1, thickness=1)
                thickness = 2
                org = (20, 30)
                org1 = (20, 60)
                org2 = (20, 90)
...
def count_car(box, w, h, id, im0):
    ...
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (190) and int(box[1]+(box[3]-box[1])/2) < (290):
...
def inputdata(car1, truck1, count1):
    ...
    truck = str(truck1)
    count = str(count1)
    kondisi = 'Malam, Terang'
    # membuat query untuk insert data ke mysql
    # sql = "INSERT INTO on_ramp_ablam(date, Mobil, Bus_Truk, total) VALUES (now(), '" + \
    #     car+"', '"+truck+"', '"+count+"')"
    sql_counting = "INSERT INTO table_testing_akurasi(date, Mobil, Bus_Truk, total, kondisi) VALUES (now(), '" + \
        car+"', '"+truck+"', '"+count+"', '"+kondisi+"')"
