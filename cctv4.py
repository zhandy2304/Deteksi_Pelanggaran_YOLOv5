    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if int(output[1]+(output[3]-output[1])/2) > (160) and int(output[1]+(output[3]-output[1])/2) < (290) and int(output[0]+(output[2]-output[0])/2) < (400) and int(output[0]+(output[2]-output[0])/2) > (100):
                            annotator.box_label(
            ...
            if show_vid:
                ...
                start_point1 = (60, 275)
                end_point1 = (400, 295)
                start_point2 = (210, 150)
                end_point2 = (60, 275)
                start_point3 = (400, 160)
                end_point3 = (400, 295)
...
def count_obj1(box, w, h, id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (160) and int(box[1]+(box[3]-box[1])/2) < (290):
        if id not in data:
            count1 = truck1 + car1
            data.append(id)
...
def count_car(box, w, h, id, im0):
    ...
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (160) and int(box[1]+(box[3]-box[1])/2) < (290) and int(box[0]+(box[2]-box[0])/2) < (400) and int(box[0]+(box[2]-box[0])/2) > (100):
        cv2.putText(im0, 'Cordinate = ' + str(int(box[0]+(box[2]-box[0])/2))+','+str(int(box[1]+(box[3]-box[1])/2)), (20,80), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, (100, 255 ,160), 2, cv2.LINE_AA)
...
def count_pelanggaran(box, w, h, id, dir2, im0, gambar, simpan):
    ...
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (140) and int(box[1]+(box[3]-box[1])/2) < (330) and int(box[0]+(box[2]-box[0])/2) < (400) and int(box[0]+(box[2]-box[0])/2) > (100):
        if id not in data2:
            person += 1
            data2.append(id)
            jp = 'Ada Orang'
            ...
            # body = """
            # Pelanggaran di Off Ramp Ablam 1"""
            # # SMPTP
