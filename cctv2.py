    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                fontScale = 1
                cv2.putText(im0, 'Total = ' + str(count3), org, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'Mobil = ' + str(car3), org1, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'Truk dan bus = ' + str(truck3), org2, font,
                            fontScale, color, thickness, cv2.LINE_AA)
...
def count_obj1(box, w, h, id):
    global count3, data, t7, t6, truck3, car3, person
    center_coordinates = (
...
def count_pelanggaran(box, w, h, id, dir2, im0, gambar, simpan):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (225) and int(box[1]+(box[3]-box[1])/2) < (285) and int(box[0]+(box[2]-box[0])/2) < (410) and int(box[0]+(box[2]-box[0])/2) > (270):
        if id not in data2:
            ...
            waktu3 = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            inputpelanggaran(jp, gambar, dir2, simpan)
            
            print("-------------------------------------------------------")
            print("Selain Kendaraan Roda 4 Memasuki Tol - On Ramp Boulevard")
            ...
            try : 
                ...
                print("Notifikasi Whatsapp berhasil dikirim pada "+str(waktu3))
                    
                for x in receiver_id:
                    bot.sendPhoto(x, photo=open(dir2, 'rb')) # send message to telegram
                    bot.sendMessage(x, ' Selain Kendaraan Roda 4 Memasuki Tol di On Ramp Boulevart') 
...
# def inputdata(car3, truck3, count3):
#     car = str(car3)
#     truck = str(truck3)
#     count = str(count3)
#     lokasi = 'On Ramp Boulevart'
